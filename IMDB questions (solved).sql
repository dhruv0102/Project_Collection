...
WHERE  Monthname(m.date_published) = 'MARCH'
       AND m.year = 2017
...
(
       SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''));SELECT     n.name                                                                           AS actor_name,
...
WITH prod_company_ranking_list
...
SELECT *
FROM   prod_company_ranking_list
...
SELECT NAME                               AS actress_name,
       ...
       Dense_rank()
         OVER(
           ORDER BY Avg(avg_rating) DESC) AS actress_rank
FROM   names n
       INNER JOIN role_mapping rm
               ON n.id = rm.name_id
       INNER JOIN ratings r
